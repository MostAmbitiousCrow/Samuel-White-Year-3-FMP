...
public class Boat_Character : MonoTimeBehaviour //, IBoatSpaceMovement
// Boat Characters are any characters that are able to interact and move with the players boat
{
    ...
    protected Boat_Character targetedCharacter;

    /// <summary> Vaults the character over the boat wall and onto a targetted character </summary>
    public void VaultOnCharacter(SpaceData spaceData, Boat_Character target)
    {
        if (!Boat_Space_Manager.Instance.CheckSpaceAccess(CanAccessOuterSides, CanAccessBoatSides, spaceData))
        {
            return;
    ...
    public void Died()
    {
        //TODO: Death Event / Game Over, Effect and Animation
        return;
    }
