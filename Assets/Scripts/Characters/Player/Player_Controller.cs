...
class Player_Controller : Boat_Character
{
    /*
    #region Variables
    [Header("Player Stats")]
    ...
    private void OnVault(InputAction.CallbackContext context)
    {
        ...
        else
        {
            ...
            CharacterStateController bc = CharacterSpaceChecks.ScanAreaForDamageableCharacter
                (newSpace.t.position, Vector3.one, Quaternion.identity, _enemyMask, true, false);
            if (bc != null)
            {
    ...
    public IEnumerator VaultJump()
    {
        ...
        CharacterStateController bc = CharacterSpaceChecks.ScanAreaForDamageableCharacter
            (_currentMoveTarget.position, new(1, 10, 1), Quaternion.identity, _enemyMask, true, false); // Scan in a tall area
        ...
        while (currentHeight > targetHeight)
        {
            ...
            float f = _fallRate * Time.fixedDeltaTime * GameManager.GameLogic.GamePauseInt;
            currentHeight -= f;
